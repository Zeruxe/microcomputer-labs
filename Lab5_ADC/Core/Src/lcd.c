...
#include "lcd.h"
#include <string.h> //imported this for the lcd_putstr function
...
void TextLCD_Position	(TextLCDType * hlcd, int col, int row)
{
	...
	{
		data_sent = 0x80 + col + 0x40;
	}

	TextLCD_SendByte(hlcd, data_sent, RW);
	My_Delay(37);
}

void TextLCD_PutChar	(TextLCDType * hlcd, char c)
{
	TextLCD_SendByte(hlcd, c, BT);
	My_Delay(37);
}
...
void TextLCD_PutStr		(TextLCDType * hlcd, char * str)
{
	...
	{
		TextLCD_SendByte(hlcd, *(str + i), BT);
		My_Delay(37);
	}
